...
class DataTransform:
    ...
    def convert_to_datetime(self, columns: List[str]):
    ...
    for col in columns:
        if self.df[col].min() > 0:
            try:
                ...
                print(f"Boxcox failed for {col}. Error: {e}")
                try:
                    self.df[col], _ = stats.yeojohnson(self.df[col])
                except Exception as e:
                    print(f"Yeo-Johnson failed for {col}. Error: {e}")
...
if __name__ == "__main__":
    ...
    save_data_to_csv(data_frame, 'loan_payments_transformed_corrected_skew.csv')
